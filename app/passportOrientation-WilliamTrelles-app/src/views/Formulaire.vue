...
export default {
  ...
  methods: {
    validateData() {
      ...
      if (!this.note1 || this.note1 > 6 || this.note1 < 0)
        this.errors.note1 = 'La note est incorrecte.'
      if (!this.note2 || this.note2 > 6 || this.note2 < 0)
        this.errors.note2 = 'La note est incorrecte.'
      if (!this.note3 || this.note3 > 6 || this.note3 < 0)
        this.errors.note3 = 'La note est incorrecte.'
      if (!this.note4 || this.note4 > 6 || this.note4 < 0)
        this.errors.note4 = 'La note est incorrecte.'
...
<template>
  <form action="" @submit.prevent="onSubmit">
    ...
    <select id="class" v-model="classe">
      ...
      <option value="CIN1D">CIN1D</option></select
    ><br />
    ...
    <textarea
      ...
      cols="50"
      v-model="comment"
      placeholder="A remplir par le/les représentant(s) légal(égaux).
        Si vous ête majeur remplissez le vous-même."
    ></textarea>
